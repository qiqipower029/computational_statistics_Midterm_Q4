...
```{r simulation}
# Create function to calculate CR for rhos
CR_cal_rho = function(r, N, true) {
  ...
  eval = as.numeric(true>=CI_low & true <= CI_high)
  print(eval)
  CR = sum(eval)/N
  return(CR)
}
# Create function to calculate CR for other parameters
CR_cal_para = function(CI, N, true) {
  eval = as.numeric(true>=CI[1] & true<=CI[2])
  CR = sum(eval)/N
}
...
simulation.function = function(parameteters, N, n.sim = 1000) {
  ...
  # Create vectors to store confidence intervals
  lambda1_ci = c()
  lambda2_ci = c()
  p1_ci = c()
  p2_ci = c()
  ...
  rho12_ci = numeric(n.sim)
  rho13_ci = numeric(n.sim)
  rho14_ci = numeric(n.sim)
  rho15_ci = numeric(n.sim)
  rho16_ci = numeric(n.sim)
  rho23_ci = numeric(n.sim)
  rho24_ci = numeric(n.sim)
  rho25_ci = numeric(n.sim)
  rho26_ci = numeric(n.sim)
  rho34_ci = numeric(n.sim)
  rho35_ci = numeric(n.sim)
  rho36_ci = numeric(n.sim)
  rho45_ci = numeric(n.sim)
  rho46_ci = numeric(n.sim)
  rho56_ci = numeric(n.sim)
  ...
  for (i in 1:n.sim) {
    ...
    lambda1_ci[i] = c(lambda1.mean - 1.96*sqrt(lambda1.mean/N), lambda1.mean + 1.96*sqrt(lambda1.mean/N))
    lambda2_ci[i] = c(lambda2.mean - 1.96*sqrt(lambda2.mean/N), lambda2.mean + 1.96*sqrt(lambda2.mean/N))
    p1_ci[i] = c(p1.mean-1.96*sqrt(p1.mean*(1-p1.mean)/N), p1.mean+1.96*sqrt(p1.mean*(1-p1.mean)/N))
    p2_ci[i] = c(p2.mean-1.96*sqrt(p2.mean*(1-p2.mean)/N), p2.mean+1.96*sqrt(p2.mean*(1-p2.mean)/N))
    t11_ci[i] = c(t11.mean-1.96*sqrt(t11.mean*(1-t11.mean)/N^2), t11.mean+1.96*sqrt(t11.mean*(1-t11.mean)/N^2))
    ...
    t21_ci[i] = c(t21.mean-1.96*sqrt((prop01*(1-prop01)+prop11*(1-prop11)-2*prop01*prop11)/N^2),
                  t21.mean+1.96*sqrt((prop01*(1-prop01)+prop11*(1-prop11)-2*prop01*prop11)/N^2))
    t31_ci[i] = c(t31.mean-1.96*sqrt((prop01*(1-prop01)+prop11*(1-prop11)+prop21*(1-prop21)-2*prop01*prop11-2*prop11*prop21-2*prop01*prop21)/N^2),
                  t31.mean+1.96*sqrt((prop01*(1-prop01)+prop11*(1-prop11)+prop21*(1-prop21)-2*prop01*prop11-2*prop11*prop21-2*prop01*prop21)/N^2))
    t12_ci[i] = c(t12.mean-1.96*sqrt(t12.mean*(1-t12.mean)/N^2), t12.mean+1.96*sqrt(t12.mean*(1-t12.mean)/N^2))
    t22_ci[i] = c(t22.mean-1.96*sqrt((prop02*(1-prop02)+prop12*(1-prop12)-2*prop02*prop12)/N^2),
                  t22.mean+1.96*sqrt((prop02*(1-prop02)+prop12*(1-prop12)-2*prop02*prop12)/N^2))
    t32_ci[i] = c(t32.mean-1.96*sqrt((prop02*(1-prop02)+prop12*(1-prop12)+prop22*(1-prop22)-2*prop02*prop12-2*prop12*prop22-2*prop02*prop22)/N^2),
                  t32.mean+1.96*sqrt((prop02*(1-prop02)+prop12*(1-prop12)+prop22*(1-prop22)-2*prop02*prop12-2*prop12*prop22-2*prop02*prop22)/N^2))    
  ...
  # Calculate CR
  lambda1_CR = CR_cal_para(lambda1_ci, n.sim, parameteters$lambda.vec[1])
  lambda2_CR = CR_cal_para(lambda2_ci, n.sim, parameteters$lambda.vec[2])
  p1_CR = CR_cal_para(p1_ci, n.sim, parameteters$prop.vec[1])
  p2_CR = CR_cal_para(p2_ci, n.sim, parameteters$prop.vec[2])
  t11_CR = CR_cal_para(t11_ci, n.sim, unlist(parameteters$prop.list[1])[1])
  t21_CR = CR_cal_para(t21_ci, n.sim, unlist(parameteters$prop.list[1])[2])
  t11_CR = CR_cal_para(t11_ci, n.sim, unlist(parameteters$prop.list[1])[3])
  t12_CR = CR_cal_para(t11_ci, n.sim, unlist(parameteters$prop.list[2])[1])
  t22_CR = CR_cal_para(t11_ci, n.sim, unlist(parameteters$prop.list[2])[2])
  t32_CR = CR_cal_para(t11_ci, n.sim, unlist(parameteters$prop.list[2])[3])
  
  rho12_CR = CR_cal_rho(rho12_s, n.sim, parameteters$corr.mat[1,2])
  rho13_CR = CR_cal_rho(rho13_s, n.sim, parameteters$corr.mat[1,3])
  rho14_CR = CR_cal_rho(rho14_s, n.sim, parameteters$corr.mat[1,4])
  rho15_CR = CR_cal_rho(rho15_s, n.sim, parameteters$corr.mat[1,5])
  rho16_CR = CR_cal_rho(rho12_s, n.sim, parameteters$corr.mat[1,6])
  rho23_CR = CR_cal_rho(rho23_s, n.sim, parameteters$corr.mat[2,3])
  rho24_CR = CR_cal_rho(rho24_s, n.sim, parameteters$corr.mat[2,4])
  rho25_CR = CR_cal_rho(rho12_s, n.sim, parameteters$corr.mat[2,5])
  rho26_CR = CR_cal_rho(rho12_s, n.sim, parameteters$corr.mat[2,6])
  rho34_CR = CR_cal_rho(rho34_s, n.sim, parameteters$corr.mat[3,4])
  rho35_CR = CR_cal_rho(rho35_s, n.sim, parameteters$corr.mat[3,5])
  rho36_CR = CR_cal_rho(rho36_s, n.sim, parameteters$corr.mat[3,6])
  rho45_CR = CR_cal_rho(rho45_s, n.sim, parameteters$corr.mat[4,5])
  rho46_CR = CR_cal_rho(rho46_s, n.sim, parameteters$corr.mat[4,6])
  rho56_CR = CR_cal_rho(rho56_s, n.sim, parameteters$corr.mat[5,6])
...
simulation.function(para1, 100, n.sim = 2)looooo```
